<template>
  <!-- @vuese
  This template renders the login page of the application. It includes a login popup for error messages, a logo, a text slider, and a form for login credentials. The form supports both manual login and Google OAuth. -->
    ...
    <div class="login-page">
      ...
      <div class="login-entry">
        <form class="login-container">
          ...
          <input id="email-entry" :style="{ fontSize: LogPassFontMod }" type="text" v-model="email"
            placeholder="email@example.com" />
          <br />
          <label id="pswd-label" :style="{ fontSize: LogPassFontMod }" for="pswd">Password:</label>
          <input id="pswd-entry" :style="{ fontSize: LogPassFontMod }" type="password" v-model="pswd"
...
export default {
  /**
   * @vuese
   * All components for the Login Page
   */
  components: {
    LoginPopup,
    TextSlider
  ...
  setup() {
    ...
    /**
     * @vuese
     * Handles the manual login process by verifying credentials and routing the user to the appropriate interface based on their role.
     */
    const login = async () => {
      try {
    ...
    /**
     * @vuese
     * Computes the font size for login and password labels dynamically based on a global text modifier.
     */
