...
export default {
  /**
   * @vuese
   * This component handles the functionality of a restaurant's ordering system,
   * including fetching menu items, managing orders, and processing checkout.
   */
  data() {
    return {
      /**
       * @vuese
       * An array to store the ordered items.
       */
      orderedItems: [],
      /**
       * @vuese
      ...
      /**
       * @vuese
       * An array of filtered menu items based on the selected category.
       */
      filteredMenuItems: [],
      /**
       * @vuese
      ...
      /**
       * @vuese
       * An array to store response data from category API calls.
       */
      respond: [],
      /**
       * @vuese
       * An array to store response data from menu item API calls.
       */
      respondItems: [],
      /**
       * @vuese
       * A counter to assign unique IDs to ordered items.
       */
      itemsID: 0,
      /**
       * @vuese
       * An object representing the menu item currently selected by the user. Contains details like id, name, price, category, and customization options like size, sugar level, etc.
       */
      selectedItem: {
        id: -1,
        name: '',
  ...
  created() {
    // Fetches categories and menu items from the API when the component is created.
  ...
  methods: {
    // Defines various methods for fetching data, manipulating the order, filtering items, etc.
    ...
    addItemToOrder(selectedSize, selectedTemperature, selectedSugarLevel, selectedIceLevel, selectedToppings) {
      // Adds a customized item to the order.
    ...
    removeItemFromOrder(orderItem) {
      // Removes an item from the order or decreases its quantity.
    ...
    removeDuplicates(menuItems) {
      // Removes duplicates from menuItems
      let noDupMenu = [];
      let UniqueItems = {};
  ...
  computed: {
    getCategoryNameById() {
      // Returns the name of a category based on its ID.
      return (categoryId) => {
    ...
    itemCost() {
       // Computes the total cost of ordered items before tax.
    ...
    categories() {
      // Returns a list of categories derived from the response data.
    ...
    menuItems() {
      // Returns a list of menu items derived from the response data, with duplicates removed and cleaned names.
    ...
    taxCost() {
      // Calculates the total tax cost for the order.
    ...
    totalCost() {
      // Calculates the total cost for the order, including tax.
      const total = this.orderedItems.reduce((acc, item) => {
        let itemCost = item.price * item.quantity;
        let toppingsCost = item.toppings.reduce((toppingAcc, topping) => {
          return toppingAcc + (topping.price * topping.quantity);
